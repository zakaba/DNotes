...
// Copyright (c) 2011-2013 DNotes Developers.
...
uint256 hashGenesisBlock("0x20bbf1cb1115ccda06c0cce115f5b72a78055a163892575988d1c8ee085f5e86");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16);
static CBigNum bnStartingDifficulty(~uint256(0) >> 26);
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight > 250 && nHeight < 33250)
		nSubsidy = 1000 * COIN;
	
	if (nHeight > FORK_HEIGHT) // 150,000
		nSubsidy = 25 * COIN;
		
	// Reward reduces 5% yearly	
	if (nHeight < 537500) 
	{
		int yearsElapsed = nHeight / 525949;
		int64 nReductionPercentage = pow(0.95,yearsElapsed);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal DNotes miner
...
bool CBlock::CheckBlock() const
{
    ...
    // Check timestamp
	if (GetBlockTime() >= FORK_TIME && GetBlockTime() > GetAdjustedTime() + 20 * 60)
		return error("CheckBlock() : block timestamp too far in the future");
    else if (GetBlockTime() < FORK_TIME && GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60)
        return error("CheckBlock() : block timestamp too far in the future");
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DNotes", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
// Genesis block
        const char* pszTimestamp = "2/16/2014 - A Florida jury convicted Michael Dunn on four charges related to his shooting into an SUV";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b5714813115273a109cff99907ba4a05d951873dae7acb6c973d0c9e7c88911a3dbc9aa600deac241b91707e7b4ffb30ad91c8e56e695a1ddf318592988afe0a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1392566028;
		...
		assert(block.hashMerkleRoot == uint256("0xe7a196e6cc384c5b43f2f1ee8ea0d89aac7f7b62f8206a3a71eb8f4a80c3dfca"));
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc0, 0xae, 0xc9 }; 
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // DNotesd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
